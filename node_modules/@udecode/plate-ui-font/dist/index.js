...
const ColorPickerToolbarDropdown = ({
  ...
  id = plateCommon.useEventPlateId(id);
  const editor = plateCommon.usePlateEditorState(id);
  const editorRef = plateCommon.usePlateEditorRef(id);
  const [open, setOpen] = React.useState(false);
  const type = plateCommon.getPluginType(editorRef, pluginKey);
  ...
  const updateColor = React.useCallback(value => {
    if (editorRef && editor && editor.selection) {
      ...
      plateCommon.select(editorRef, editor.selection);
      plateCommon.focusEditor(editorRef);
